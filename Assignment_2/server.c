...
int main(int argc, char const *argv[])
{
	...
	if (child_pid == 0)
	{
        ...
        struct passwd *password = getpwnam("nobody");             //obtain user ID
           	...
           	sprintf(file_name, "%s", argv[0]);                     //get file name
	    	sprintf(socket_name, "%d", new_socket);                //get socket name

	    	char *args[] = {file_name, socket_name, NULL};
	    	if(execvp(file_name, args) == -1)                       //give priviliges
            {
                printf("Exect Failed\n");
                exit(EXIT_FAILURE);
            }                               
